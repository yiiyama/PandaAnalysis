...
presel = 'nJet>=1 && jet1Pt>100'
cuts = {
    # analysis regions
    'signal'            : tAND(metFilter,tAND(presel,'pfmet>250 && dphipfmet>0.5 && (nLooseMuon+nLooseElectron+nLoosePhoton+nTau)==0 && fabs(calomet-pfmet)/pfmet<0.5')), # turning off btags
    #'signal'            : tAND(metFilter,tAND(presel,'pfmet>250 && dphipfmet>0.5 && (nLooseMuon+nLooseElectron+nLoosePhoton+nTau)==0 && fabs(calomet-pfmet)/pfmet<0.5')), # turning off btags
...
#    'singleelectronw'   : tAND(metFilter,tAND(presel,'pfUWmag>250 && (nLooseMuon+nLoosePhoton+nTau)==0 && nLooseElectron==1 && looseLep1IsTight==1 && fj1MaxCSV<0.46 && isojetNBtags==0 && pfmet>40')),
    'singlemuon'        : tAND(metFilter,tAND(presel,'pfUWmag>250 && (nLooseElectron+nLoosePhoton+nTau)==0 && nLooseMuon==1 && looseLep1IsTight==1 && fabs(calomet-pfmet)/pfUWmag<0.5')),
    'singleelectron'    : tAND(metFilter,tAND(presel,'pfUWmag>250 && (nLooseMuon+nLoosePhoton+nTau)==0 && nLooseElectron==1 && looseLep1IsTight==1 && pfmet>40 && fabs(calomet-pfmet)/pfUWmag<0.5')),
    'dimuon'            : tAND(metFilter,tAND(presel,'pfUZmag>250 && (nLooseElectron+nLoosePhoton+nTau)==0 && nLooseMuon==2 && looseLep1IsTight==1 && fabs(calomet-pfmet)/pfUZmag<0.5')),
    'dielectron'        : tAND(metFilter,tAND(presel,'pfUZmag>250 && (nLooseMuon+nLoosePhoton+nTau)==0 && nLooseElectron==2 && looseLep1IsTight==1 && fabs(calomet-pfmet)/pfUZmag<0.5')),
    'photon'            : tAND(metFilter,tAND(presel,'pfUAmag>250 && (nLooseMuon+nLooseElectron+nTau)==0 && nLoosePhoton==1 && loosePho1IsTight==1 && fabs(calomet-pfmet)/pfUAmag<0.5')),
}

weights = {
    # analysis weights
#  'signal_nobtag'    : tTIMES('%f*normalizedWeight*sf_pu*sf_lep*sf_ewkV*sf_qcdV*sf_tt8TeV',topTagSF),
  'signal'    : tTIMES('%f*normalizedWeight*sf_ewkV*sf_qcdV*sf_tt8TeV',topTagSF),
  ...
  'notag'     : tTIMES('%f*normalizedWeight*sf_ewkV*sf_qcdV*sf_tt8TeV',topTagSF),
  'singleelectron' : tTIMES('%f*normalizedWeight*sf_ewkV*sf_qcdV*sf_tt8TeV',topTagSF),
  'singlemuon'     : tTIMES('%f*normalizedWeight*sf_ewkV*sf_qcdV*sf_tt8TeV',topTagSF),
}
