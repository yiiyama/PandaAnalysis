...
jesr_sigma = {x:root.RooRealVar('jesr_sigma%i'%x,'jesr_sigma%i'%x,0.5,0.05,5) for x in [0,1,3]}
jesr_mu = {x:root.RooRealVar('jesr_mu%i'%x,'jesr_mu%i'%x,0,-5,5) for x in [0,1,3]}
#jesr_mu[3] = root.RooRealVar('jesr_mu%i'%3,'jesr_mu%i'%3,0,-10,10)
jesr = {x:root.RooGaussian('jesr%i'%x,'jesr%i'%x,mass,jesr_mu[x],jesr_sigma[x]) for x in [0,1,3]}
#jesr = {x:root.RooGaussModel('jesr%i'%x,'jesr%i'%x,mass,jesr_mu[x],jesr_sigma[x]) for x in [0,1,3]}
for iP in [3,0]:
  for iC in [0,1]:
    cat = (iP,iC)
#    if iP==3:
    if False:
      smeared[cat] = root.RooFFTConvPdf('conv%i%i'%cat,'conv%i%i'%cat,mass,pdfprong[cat],jesr[0])
    else:
...
for iC in [0,1]:
  ...
  plot[iC].AddPlotLabel('#varepsilon_{tag}^{Data} = %.3g#pm%.2g'%(eff_post,eff_posthi),
                        .6,.47,False,42,.04)
  #plot[iC].AddPlotLabel('#varepsilon_{tag}^{MC} = %.3g^{+%.2g}_{-%.2g}'%(eff_,err_,err_),
  plot[iC].AddPlotLabel('#varepsilon_{tag}^{MC} = %.3g'%(eff_),
                        .6,.37,False,42,.04)
  plot[iC].AddPlotLabel('#varepsilon_{tag+mSD}^{Data} = %.3g#pm%.2g'%(effMass*eff_post,effMass*eff_posthi),
                        .6,.27,False,42,.04)
...
# save outpuat
rcn = root.RooFit.RecycleConflictNodes
...
for x in [nsigtotal,eff,sample,datacomb,simult]:
  w.imp(x,rcn())
for iC in [0,1]:
  w.imp(normsig[iC],rcn())
  w.imp(dh_data[iC],rcn())
  w.imp(model[iC],rcn())
  for iP in [0,3]:
    cat = (iP,iC)
#    w.imp(smear[iP]); w.imp(mu[iP])
    w.imp(dhprong[cat],rcn())
    w.imp(pdfprong[cat],rcn())
    w.imp(norm[cat],rcn())
#    w.imp(smeared[cat])
